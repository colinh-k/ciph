...
#define Nr (10 + (Nk - 4))
// block size (fixed to be 4 in this standard) (in bytes) (number of columns in the state)
#define Nb 4
...
// get a byte at index i from a word
// #define WGET_B(w, i) ((w) >> (sizeof(byte) * (i)) & 0xFF)
// set a byte at index i from a word
// #define WSET_B(w, i)
...
Word SubWord(Word word) {
    // byte i0 = (word >> (8 * 0)) & 0xFF;
    Word ret = 0;
    for (size_t i = 0; i < sizeof(Word); i++) {
        Word value = sbox[WLOC(word, i)];
        value <<= 8 * i;
        WLOC(word, i) = sbox[WLOC(word, i)];
    }
}

Word RotWord(Word word) {
    // TODO: may be accomplished with memmove()
    // https://stackoverflow.com/questions/12633039/shift-elements-in-array
    // if (word->nBytes <= 1) {
    //     return;
    // }

    // for (size_t i = 0; i < word->nBytes - 1; i++) {
    //     byte temp = WLOC(word, i);
    //     WLOC(word, i) = WLOC(word, i + 1);
    //     WLOC(word, i + 1) = temp;
    // }
}
